# api-sync-triggers Specification

## Purpose

`POST /sync/**` endpoints that trigger TimeCamp, Jira, and worklog sync tasks.
Each trigger is wrapped in the `api_jobs` lifecycle, returns `job_id` plus
current status, supports background execution, and requires authentication.
## Requirements
