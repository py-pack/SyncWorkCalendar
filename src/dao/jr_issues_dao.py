...
class JRIssuesDAO(BaseDAO):
    ...
    async def sync_by_key(self, issues: List[JiraIssueDTO]):
        ...
        service = JRUsersDAO()
        await service.update_by_keys(users_dto, key_sync='key')

        projects_dto = [project_dto for project_dto in projects.values()]
        service_project = JRProjectDAO()
        await service_project.update_by_keys(projects_dto, key_sync='key')

        service_jira_issue = JRIssuesDAO()
        await service_jira_issue.update_by_keys(issues)
